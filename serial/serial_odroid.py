...
# This program is used to test the communication between the button controller via a USB serial connection.

import serial.tools.list_ports
from threading import Thread
import time

# get all the serial comports
ports = serial.tools.list_ports.comports()
# create an instance of the serial method
...
def identify(address):
    ...
    device = serial.Serial(address, 115200, timeout=.1)
    global buttonStates
    while True:
...
# create and array with the connected serial devices
...
# method to keep checking for changes
...
# create a thread for each connected serial device
for address in portAddress:
    address = Thread(target=identify, args=[address])
    address.start()

# create a thread for the buttonpresscheck
